...
#
# Tags look like `v<FMD Server version>-<release>`, which is what this
# repository has always published (v0.11.0-1 ... v0.16.0-1):
#
# - if defaults/main.yml points at an FMD Server version that has never been
#   released, the release counter restarts at 0 (`v0.17.0-0`)
# - otherwise the counter is incremented (`v0.17.0-1`), but only if something
...
# The workflow this replaced looked for a Renovate commit whose subject said
# "docker tag to", which is not a subject Renovate has produced for this
# repository in a long time - so it never released anything, and every tag
# since had to be pushed by hand.
...
# Anchored on `fmd_server_version:` so that neither a commented-out example nor
# `fmd_server_container_image_tag`, which is derived from it, can be mistaken
# for it.
version="$(sed -nE 's|^fmd_server_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the FMD Server version from $defaults_path"
	exit 1
fi

# FMD Server's own version is carried without a leading `v` (the `v` lives in
# the tags this repository publishes, and in
# `fmd_server_container_image_self_build_repo_version`), but tolerate one so
